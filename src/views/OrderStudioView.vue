...
const { customerName, phone, address, color, size, quantity, unitPrice, totalPrice } =
  storeToRefs(orderStore)
const MAIL_TO = 'rronaliu95@gmail.com'

const activeShirt = computed(() => shirtColors.find((shirt) => shirt.id === color.value) ?? shirtColors[0])
...
const orderSummary = computed(() => {
  return [
    ...
    `Design file: ${designFile.value ? designFile.value.name : 'No file attached'}`,
  ].join('\n')
})
...
<template>
  <main class="order-studio">
    ...
    <section class="content-grid">
      ...
      <OrderSetupPanel :shirt-colors="shirtColors" :sizes="sizes" @submit="submitOrder" />
    </section>
  </main>
</template>
